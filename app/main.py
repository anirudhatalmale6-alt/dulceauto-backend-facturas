...
Las acciones de crear, editar, duplicar y generar PDF llegan en las fases
siguientes. Donde todavia no estan cableadas, la pantalla lo dice con una
etiqueta en lugar de ofrecer un boton que no hace nada.
"""
...
def invoice_new(request: Request, db: Session = Depends(get_db)):
    ...
    return render(
        ...
        page_sub="Una sola pantalla operativa para las 3 plantillas aprobadas.",
        invoice=None,
...
def invoice_edit(request: Request, invoice_id: int, db: Session = Depends(get_db)):
    ...
    return render(
        ...
        active_view="editor",
        page_title="Crear / Editar",
        page_sub=f"Factura {invoice.folio} · {get_market(invoice.locale).label}",
        invoice=invoice,
        editable=EDITABLE_FIELDS,
    )
